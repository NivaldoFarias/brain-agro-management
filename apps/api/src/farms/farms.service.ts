...
export class FarmsService {
	...
	async countByState(): Promise<Array<{ state: string; count: number }>> {
		const results = await this.farmRepository
			.createQueryBuilder("farm")
	...
	async getLandUseStats(): Promise<{ arableArea: number; vegetationArea: number }> {
		...
		return {
			arableArea: Number.parseFloat(result.arableArea) || 0,
			vegetationArea: Number.parseFloat(result.vegetationArea) || 0,
		};
	}
	...
	private async verifyProducerExists(producerId: string): Promise<void> {
		const producerExists = await this.producerRepository.exists({
			where: { id: producerId },
		});
